...
const getFullPath = (path: string) => {
  const serverUrl =
    process.env.GATSBY_CDN_URL_APP || "https://images.gostudy.cz/app";
...
export const buildImageSrcByProps = (props: ImageProps): string => {
  const realpath =
    props.path.indexOf("http") === 0 ? props.path : getFullPath(props.path);
  ...
  switch (props.template) {
    ...
    default:
      return `${realpath}${
        props.customParams ? `?tx=${props.customParams}` : ""
      }`;
  }
};

export const buildAccommodationImg = (
  path: string,
  alt: string
): React.ReactElement => {
...
export const getSrapiImageSrc = (
  ...
  return buildImageSrcByProps({
    path: `${cdnurlStrapi}${imgName}`,
    customParams: `c_fill,h_${height},w_${width}`,
  });
};
...
export const getSrapiImageSrcSet = (
  cdnurlStrapi: string,
  imgUrl: string,
  width: number,
  height: number
...
export const getPicture = (
  ...
  const sources = [1, 2, 3, 4].map((resolution) =>
