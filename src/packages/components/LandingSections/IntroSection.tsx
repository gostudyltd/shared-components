...
import { HorizontalListWithSeparator, HorizontalListWithSeparatorProps } from './components/HorizontalListWithSeparator'
...
export const IntroSection: React.FC<IntroSectionProps> = (props) => {
  const { title, renderTitle, description, image, button, accentColor = 'primary', preTitle, bottomListData } = props
  ...
  return (
    <SectionContainer
      ...
      sx={{
        position: 'relative',
        padding: bottomListData ? { xs: '0 0 8.25rem 0', sm: '0 0 3.75rem 0' } : undefined,
      }}
    >
      <SectionTwoColumnsContainer
            ...
            <Stack gap={{ xs: '1em', sm: '1.5em' }}>
              {preTitle && (
                ...
                >
                  ...
                  <Typography
                    variant='h5'
                    sx={{ color: accent.main, fontSize: '1.25rem', fontWeight: 600, transition: 'all .2s linear' }}
                  >
                    {preTitle}
                  </Typography>
                </Stack>
              )}
              <Typography variant='h1' fontSize={'3rem'} display={{ xs: 'none', sm: 'block' }} fontWeight={600}>
                {renderTitle ? renderTitle(accent.main) : title}
              </Typography>
              <Typography variant='h2' display={{ xs: 'block', sm: 'none' }} fontWeight={600}>
                {renderTitle ? renderTitle(accent.main) : title}
              </Typography>
        ...
        secondColumn={<Box component={'img'} src={image} alt='hero' width={'100%'} maxWidth={'588px'} mx={'auto'} />}
      />
      {bottomListData && (
