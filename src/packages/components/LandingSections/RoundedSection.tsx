...
import { AccentColor, SectionContainerProps, SectionTitleProps } from './components/types'
...
const getBackgroundByAccentColor = (accentColor: AccentColor, revertGradient?: boolean): string => {
  if (accentColor === 'warning')
    return `linear-gradient(${revertGradient ? '-239.72deg' : '282.72deg'}, #FFEDD1 31.93%, #FFF3E0 31.94%)`
  // if (accentColor === 'error') return 'linear-gradient(282.72deg, #FFEDD1 31.93%, #FFF3E0 31.94%)'
  return `linear-gradient(linear-gradient(${revertGradient ? '-239.72deg' : '282.72deg'}, #D3EEFF 31.93%, #E1F3FF 31.94%)`
}
  ...
  return (
    <SectionContainer {...sectionContainer}>
      <Stack
        ...
        width={'100%'}
        sx={{
          background: getBackgroundByAccentColor(accentColor, revertGradient),
      ...
      >
        ...
        >
          <Stack direction={'column'} flex={'1 1 0'}>
            <SectionTitle
              ...
              {...sectionTitle}
            />
            <LeftContent content={leftContent} button={button} sx={{ display: { xs: 'none', sm: 'flex' } }} />
          </Stack>
          <Stack
            flex={'1 1 0'}
            sx={{
        ...
        {bottomContent && (
          <Stack sx={{ position: 'relative', marginTop: { xs: '', sm: '2rem' } }}>
            {image && (
              <Box
                component={'img'}
                src={image}
                sx={{ position: { xs: 'initial', sm: 'absolute' }, bottom: '100%', right: 0 }}
              />
            )}
            {bottomContent}
          </Stack>
        ...
        <LeftContent
          content={leftContent}
          button={button}
          sx={{ display: { xs: 'flex', sm: 'none' }, marginBottom: { xs: '1.25rem' } }}
        />
      </Stack>
    </SectionContainer>
  )
