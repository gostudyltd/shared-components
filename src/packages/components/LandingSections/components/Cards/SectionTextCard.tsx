...
export const SectionTextCard1: React.FC<SectionTextCardPropsShared> = (props) => {
  const { sx, accentColor = 'primary', renderText, renderTitle, text, iconName, title } = props
  ...
  return (
    <Stack sx={sx}>
      <Stack direction={'row'}>
        ...
        >
          {renderTitle ? renderTitle(title, accent) : <span style={{ color: accent }}>{title}</span>}
        </Typography>
      </Stack>
      <Box
        component={'hr'}
...
export const SectionTextCard2: React.FC<SectionTextCardPropsShared & { disableBorder?: boolean }> = (props) => {
  const { sx, accentColor = 'primary', renderText, renderTitle, text, iconName, title, disableBorder } = props
  const accent = accentColorBase[accentColor].main
  return (
    <Stack
      sx={{
...
export const SectionTextCard4: React.FC<SectionTextCardPropsShared> = (props) => {
  const { sx, accentColor = 'primary', renderText, renderTitle, renderIcon, text, iconName, title } = props
  const accent = accentColorBase[accentColor].main
...
export const SectionTextCard: React.FC<SectionTextCardProps> = ({ variant, ...restProps }) => {
  if (variant === 'v5') {
    return (
      <SectionTextCard4
  ...
  if (variant === 'v6') {
    ...
    return (
      <SectionTextCard4
        ...
        renderTitle={(text) => (
          <Box component={'span'} fontSize={{ xs: '12px', sm: '18px' }} width={'100%'}>
            {text}
          </Box>
        )}
        renderIcon={(icon, color) => (
