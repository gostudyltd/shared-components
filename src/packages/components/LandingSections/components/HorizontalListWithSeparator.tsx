...
export const HorizontalListWithSeparator: React.FC<HorizontalListWithSeparatorProps> = ({ data, sx = {}, itemSx = {} }) => {
  return (
    <Stack
      direction={'row'}
      // alignItems={'center'}
    ...
    >
      ...
      {data.map(({ icon, text }, idx, arr) => {
        const itemMaxWIdth = data.length === 3 ? '20.125rem' : data.length === 2 ? '40.25rem' : '100%'
        ...
        return (
          <Fragment key={text}>
            <Stack
              ...
              sx={{
                width: '100%',
                maxWidth: { xs: 'inherit', sm: itemMaxWIdth, alignItems: 'center' },
                '& > img, & > svg': {
